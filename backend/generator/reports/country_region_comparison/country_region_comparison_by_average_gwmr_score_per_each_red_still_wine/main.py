...
class CountryRegionComparisonByAverageGwmrScorePerEachRedStillWine(WineryReport):
    @property
    def title(self):
        return "8.3 Country/Region comparison by average GWMR Score per each Red Still wine"
    ...
    def render(self, throw_exception=False):
        """Country/Region comparison by average GWMR Score per each Red Still wine"""
        type_report = "get_comparison_by_average_gwmr_score_per_each_red_still_wine"
        ...
        with self.database.connect() as connection:
            ...
            param_name = "Average number of medals per each red still wine"
            ...
            query = text(
                ...
                WHERE winery.id = :winery_id AND event.year BETWEEN :year_from AND :year_to AND wine_entity.gwmr IS NOT NULL\
                AND wine.color = 1169 AND JSON_VALUE(wine.category, '$.co2') = 1358"
            ...
            try:
                ...
                query = text(
                    "SELECT wine_entity.gwmr, wine_entity.id FROM wine_entity\
                    INNER JOIN wine ON wine_entity.wine=wine.id\
                    INNER JOIN award_wine_entity ON wine_entity.id=award_wine_entity.wine_entity_id\
                    INNER JOIN award ON award_wine_entity.award_id=award.id\
                    INNER JOIN event ON award.event_id=event.id\
                    WHERE (event.year BETWEEN :year_from AND :year_to) AND wine_entity.region = :region_id AND wine_entity.gwmr IS NOT NULL\
                    AND wine.color=1169 AND JSON_VALUE(wine.category, '$.co2')=1358\
            ...
            query = text(
                ...
                WHERE (event.year BETWEEN :year_from AND :year_to) AND winery.country IN\
                    ...
                    WHERE winery.id = :winery_id)\
                AND wine.color=1169 AND JSON_VALUE(wine.category, '$.co2')=1358 AND wine_entity.gwmr IS NOT NULL\
                GROUP BY award_wine_entity.wine_entity_id"
            ).bindparams(winery_id=winery, year_from=year1, year_to=year2)
