...
class CountryRegionComparisonByAverageGwmrScorePerEachWine(WineryReport):
    @property
    def title(self):
        return "8.3 Country/Region comparison by average GWMR Score per each wine"
    ...
    def render(self, throw_exception=False):
        """Country/Region comparison by average GWMR Score per each wine"""
        type_report = "get_comparison_by_average_gwmr_score_per_each_wine"
        ...
        with self.database.connect() as connection:
            ...
            query = text(
                ...
                WHERE (event.year BETWEEN :year_from AND :year_to) AND winery.country IN\
                    ...
                    WHERE winery.id = :winery_id) AND wine_entity.gwmr IS NOT NULL\
                GROUP BY award_wine_entity.wine_entity_id"
